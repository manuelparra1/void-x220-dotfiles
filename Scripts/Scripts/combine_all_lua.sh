...
echo "# Combined Lua Files" > "$output_file"
echo "" >> "$output_file"

# Find all Lua files and process them
find . -type f -name "*.lua" -print0 | while IFS= read -r -d '' file; do
  # Output separator lines and file name to output.lua
  echo "## \`${file#./}\`" >> "$output_file"
  echo "---" >> "$output_file"

  # Output contents of each file, wrapped in ```lua
